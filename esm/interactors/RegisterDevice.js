class RegisterDevice {
  ...
  async createDevice(properties, session) {
    ...
    if (!type) {
      this.generateError('Type property is required');
    }
  ...
  async createAppOrGatewayDevice(basicDevice, session) {
    ...
    if (!await this.sessionOwnerIsUser(session)) {
      this.generateError('Device owner isn\'t a user');
    }

    device.meshblu = {
      version: '2.0.0',
      whitelists: this.generateWhitelists(session.uuid),
    };
