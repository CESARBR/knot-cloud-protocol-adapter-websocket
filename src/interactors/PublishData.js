...
class PublishData {
  ...
  async execute(requestId, data) {
    ...
    await this.cloud.broadcastMessage(session.credentials, 'data', { sensor_id: data.sensorId, value: data.value });
    return { type: 'published' };
  }

  validateValueType(device, data) {
    const schema = device.schema.find(value => value.sensor_id === data.sensorId);
    ...
    switch (schema.value_type) {
      case 1: // INTEGER
        if (!(data.value instanceof Number) && !Number.isInteger(data.value)) {
