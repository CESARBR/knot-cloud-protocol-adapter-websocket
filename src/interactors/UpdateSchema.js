...
class UpdateSchema {
  ...
  async execute(id, data) {
    ...
    if (!await this.isSessionThing(session)) {
      this.generateError('Device has no type thing', 401);
    }

    try {
      if (this.isValid(data.schema)) {
        await this.cloud.updateDevice(session.credentials.uuid,
          { schema: data.schema }, session.credentials);
      }
    } catch (error) {
      throw error;
    }
    return { type: 'SchemaSent' };
  }
  ...
  async isSessionThing(session) {
    const device = await this.cloud.getDevice(session.credentials.uuid, session.credentials);
